...
function renderOpeningHourEntry (ophObj, weekDays, fallBack, hideWeekDay) {
  ...
  return html`
    <div class="pl-opening-hours-entry ${highlightClass}">
      ...
      <div class="pl-opening-hours-entry-hours">
        ${fromTill}
      </div>
    </div>
  `
...
const OpeningHours = function ({ id, actionBox }, lang, emit) {
  ...
  return html`
  <div class="pl-opening-hours">
    <div class="pl-opening-hours-heading pl-hide-on-mobile">
      ${raw(translate('openingHours', lang))}
      ${openingHoursCaption ? html`<br>` : null}
      ${openingHoursCaption}
    </div>

    <div class="pl-opening-hours-heading pl-hide-on-desktop" onclick=${() => emit('toggleOpeningHours', id)}>
      ${mobileText || raw(translate('openingHours', lang))}
      ${mobileIcon}
    </div>

    <div class="pl-opening-hours-entries ${boxOpen ? 'pl-open' : ''}">
      ${openingHourEntries}
    </div>
  </div>
  `
