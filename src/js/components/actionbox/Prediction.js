  ...
  return html`
    <div class="pl-box-time">
      <div class="pl-time-data">
        ${startTime} ${endTime ? ' - ' + endTime : ''} ${ icon } 
      </div>
      ${ timeCaption ? html`
...
const Prediction = (tHeader) => {
  const { actionBox } = tHeader
  ...
  const rerouteLink = RerouteLink(tHeader)
  ...
  return html`
      <div class="pl-box pl-action-box pl-box-calendar">
        ${ heading }
        ${ calendar }
        <div class="pl-spaced-list pl-box-body">
          ${ timeBox }
          ${ caption }
          ${ rerouteLink }
        </div>
