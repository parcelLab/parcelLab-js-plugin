...
const PickupLocation = ({ id, actionBox, last_delivery_status: lastStatus }, lang, openingHoursWarning, emit) => {
  if (!actionBox.address) return null

  const openingHours = (actionBox.data && actionBox.data.openingHours) ? OpeningHours({ id, actionBox }, lang.name, openingHoursWarning, emit) : null

  const heading = lastStatus ? html`
